...
createUserWithEmail name email brig = do
    r <- postUserWithEmail name (Just email) False Nothing Nothing brig <!!
           const 201 === statusCode
    decodeBody r
...
postUser name haveEmail havePhone ssoid teamid brig = do
    email <- if haveEmail
        then Just <$> randomEmail
        else pure Nothing
    postUserWithEmail name email havePhone ssoid teamid brig

-- | More flexible variant of 'createUserUntrustedEmail' (see above).
postUserWithEmail :: Text -> Maybe Email -> Bool -> Maybe UserSSOId -> Maybe TeamId -> Brig -> Http ResponseLBS
postUserWithEmail name email havePhone ssoid teamid brig = do
    phone <- if havePhone
